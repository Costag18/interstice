...
export function renderNav(root) {
  ...
  const markup = `
    <aside class="hidden md:flex h-screen w-64 fixed left-0 top-0 flex-col py-8 px-4 bg-background z-30 border-r border-outline-variant/10">
      <div class="mb-12 px-4">
        <h1 class="text-2xl font-headline italic text-on-surface">Interstice</h1>
        <p class="font-label text-[10px] uppercase tracking-[0.2em] text-on-surface/40 mt-1">The Analog Pause</p>
      </div>
      <nav class="flex-1 space-y-2" data-nav="side">${sidebarLinks}</nav>
      <div class="mt-auto px-4 py-3 flex items-center gap-2 text-on-surface/40">
        <span class="material-symbols-outlined text-base">lock</span>
        <span class="font-label text-[10px] uppercase tracking-widest">Saved on this device</span>
      </div>
    </aside>
...
function sideLink(it) {
  return `<a href="#/${it.name}" data-go="${it.name}" data-name="${it.name}"
    class="flex items-center gap-4 py-3 px-4 rounded-lg text-on-surface/60 hover:bg-surface-container hover:text-on-surface transition-colors">
    <span class="material-symbols-outlined">${it.icon}</span>
    <span class="font-label text-sm tracking-tight">${it.label}</span></a>`;
}
